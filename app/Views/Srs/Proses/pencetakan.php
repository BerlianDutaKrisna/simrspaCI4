...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        ...
        <!-- Form -->
        <form id="mainForm" action="<?= base_url('pencetakan_srs/proses_pencetakan'); ?>" method="POST">
            <?= csrf_field(); ?>
            <!-- Input Hidden -->
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm text-center" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php if (!empty($pencetakanDatasrs)): ?>
                            <?php $i = 1; ?>
                            <?php foreach ($pencetakanDatasrs as $row): ?>
                                <tr>
                                    <td><?= $i ?></td>
                                    <td>
                                        <input type="checkbox"
                                            name="id_proses[]"
                                            value="<?= $row['id_pencetakan_srs']; ?>:<?= $row['id_srs']; ?>:<?= $row['id_mutu_srs']; ?>"
                                            class="form-control form-control-user checkbox-item"
                                            data-status='<?= json_encode([
                                                                'status_pencetakan_srs' => $row['status_pencetakan_srs'] ?? ""
                                                            ]) ?>'
                                            autocomplete="off">
                                    </td>
                                    ...
                                    <td>
                                        <?= empty($row['selesai_pencetakan_srs']) ? '-' : esc(date('H:i , d-m-Y', strtotime($row['selesai_pencetakan_srs']))); ?>
                                    </td>
                                    <td>
                                        <?php
