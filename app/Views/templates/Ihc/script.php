...
<script>
    $(document).ready(function() {
        ...
        $('#penerimaModal').on('show.bs.modal', function(event) {
            ...
            var id_ihc = button.data('id_ihc'); // Kode ihc
            var penerima_ihc = button.data('penerima_ihc'); // Nama penerima

            var modal = $(this);
            modal.find('#id_ihc').val(id_ihc); // Isi id_ihc ke input hidden
            modal.find('#penerima_ihc').val(""); // Isi penerima_ihc dengan data dari tombol
        });
        // ==========================
        // Modal Status ihc
        // ==========================
        $('#statusihcModal').on('show.bs.modal', function(event) {
            ...
            var idihc = button.data('id_ihc'); // Extract id_ihc from data-* attributes
            var statusihc = button.data('status_ihc'); // Extract status_ihc

            var modal = $(this);
            modal.find('.modal-body #id_ihc').val(idihc); // Set id_ihc in the modal input
            modal.find('.modal-body #status_ihc').val(statusihc); // Set status_ihc in the select
        });

        // ==========================
        // Hapus DATA
        // ==========================
        $(document).on("click", ".delete-ihc, .delete-frs, .delete-pengirisan, .delete-pemotongan, .delete-pemprosesan, .delete-penanaman, .delete-pemotongan_tipis, .delete-pewarnaan, .delete-pembacaan, .delete-penulisan, .delete-pemverifikasi, .delete-autorized, .delete-pencetakan", function() {
            var action = $(this).data("action"); // Menyimpan data action (ihc atau pengirisan)
            var id_ihc = $(this).data("id_ihc");
            ...
            $("#confirmDelete").data("id_ihc", id_ihc);
        ...
        $("#confirmDelete").on("click", function() {
            ...
            if (action === "ihc") {
                url = "<?= base_url('ihc/delete'); ?>"; // URL penghapusan ihc
                data = {
                    id_ihc: id_ihc
                };
            } else if (action === "frs") {
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        if (action === "penerimaan") {
                            ...
                            detailHtml = `
                        <p><strong>Kode ihc:</strong> ${response.kode_ihc || "-"}</p>
