...
<div class="card shadow mb-4">
    ...
    <div class="card-body">
        <h1>Daftar Pewarnaan Fine Needle Aspiration Biopsy</h1>
        <a href="<?= base_url('/dashboard') ?>" class="btn btn-primary mb-3"><i class="fas fa-reply"></i> Kembali</a>
        <?= $this->include('templates/proses/button_pewarnaan'); ?>

        <!-- Form -->
        <form id="mainForm" action="<?= base_url('pewarnaan_frs/proses_pewarnaan'); ?>" method="POST">
            <?= csrf_field(); ?>
            <!-- Input Hidden -->
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm text-center" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <?php if (!empty($pewarnaanDatafrs)): ?>
                            <?php $i = 1; ?>
                            <?php foreach ($pewarnaanDatafrs as $row): ?>
                                <tr>
                                    <td><?= $i ?></td>
                                    <td>
                                        <input type="checkbox"
                                            name="id_proses[]"
                                            value="<?= $row['id_pewarnaan_frs']; ?>:<?= $row['id_frs']; ?>:<?= $row['id_mutu_frs']; ?>"
                                            class="form-control form-control-user checkbox-item"
                                            data-status='<?= json_encode([
                                                                'status_pewarnaan_frs' => $row['status_pewarnaan_frs'] ?? ""
                                                            ]) ?>'
                                            autocomplete="off">
                                    </td>
                                    <td><?= $row['kode_frs']; ?></td>
                                    ...
                                    <td><?= $row['status_pewarnaan_frs']; ?></td>
                                    <td><?= $row['nama_user_pewarnaan_frs']; ?></td>
                                    <td>
                                        <?= empty($row['mulai_pewarnaan_frs']) ? '-' : esc(date('H:i , d-m-Y', strtotime($row['mulai_pewarnaan_frs']))); ?>
                                    </td>
                                    <td>
                                        <?= empty($row['selesai_pewarnaan_frs']) ? '-' : esc(date('H:i , d-m-Y', strtotime($row['selesai_pewarnaan_frs']))); ?>
                                    </td>
                                    <td>
                                        <?php
