<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    $(".btn-cetak-stiker").on("click", function() {
      let id_srs = $(this).data("id");
      let kode = $(this).data("kode");

      // Ambil jumlah slide terbaru
      let slide = parseInt($(`.jumlah-slide-input[data-id="${id_srs}"]`).val(), 10);
      if (isNaN(slide) || slide < 1) {
        slide = 1;
      }

      // Tentukan label sesuai jumlah slide
      let labelSlides = [];
      if (slide === 1) {
        labelSlides = ["PAP"];
      } else if (slide === 2) {
        ...
        labelSlides = ["PAP", "DQ", "CB"];
      } else if (slide > 3) {
        labelSlides = ["PAP", "DQ", "CB"];
        // Tambah string kosong sesuai kelebihan slide
        for (let j = 4; j <= slide; j++) {
          labelSlides.push("");
        }
      }

      console.log("Cetak stiker untuk ID:", id_srs, "Jumlah Slide:", slide, "Label:", labelSlides);
      ...
      <head>
        ...
        <style>
          ...
          .kode-srs { font-size: 9pt; font-weight: bold; margin: 0.1mm 0; }
        ...
        <div class="label">
          <div class="inner-label">
            ...
            <div class="teks-label">
              ...
              <div class="kode-srs">${kode}</div>
              <div class="angka-romawi">${labelSlides[i] || ""}</div>
            </div>
          </div>
        </div>`;
      }
