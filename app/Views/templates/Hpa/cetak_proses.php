...
<head>
    ...
    <style>
        @page { size: 215mm 350mm; margin: 20mm; }
        body { font-family: Verdana, Arial, sans-serif; margin: 10px; padding: 0; height: 100%; display: flex; flex-direction: column; }
        h5 { margin: 5px 0; }
        table { width: 100%; table-layout: fixed; border-collapse: collapse; word-wrap: break-word; }
        td { padding: 5px; text-align: left; vertical-align: middle; border: 1px solid black; }
        hr { border: 1px solid #000; margin: 20px 0; }
        .header-table { width: 100%; border-collapse: collapse; margin: 0; font-size: 11px; font-family: Verdana, Arial, sans-serif; }
        .header-table td { padding: 3px 4px; vertical-align: top; border: 1px solid #000; line-height: 1.2; }
        .makroskopis-content-table td, .mikroskopis-content-table td { font-size: 12px; padding: 5px; }
        .makroskopis-content-table td:first-child, .mikroskopis-content-table td:first-child { width: 30%; }
        .makroskopis-content-table td:last-child, .mikroskopis-content-table td:last-child { width: 70%; }
        .makroskopis-content-table td[colspan="2"] { height: 150px; font-size: 16pt; border: 1px solid black; border-bottom: none; padding: 5px; text-align: left; vertical-align: top; }
        .no-border{ border-right: none; border-top: none; }
        .foto-makroskopis{ border-left: none; border-top: none; width: 100%; text-align: right; vertical-align: top; }
        .mikroskopis-content-table td[colspan="2"] { height: 400px; font-size: 16pt; border: 1px solid black; padding: 5px; text-align: left; vertical-align: top; }
        .mutu-table { font-size: 12px; font-family: Verdana, Arial, sans-serif; border-collapse: collapse; width: 100%; margin: 0; }
        .mutu-table td { border: 1px solid black; padding: 5px; vertical-align: middle; text-align: left; }
        .gambar-table { width: 100%; border-collapse: collapse; }
        .gambar-table td { height: 110px; width: 12.5%; border: 1px dashed black; text-align: center; vertical-align: middle; position: relative; }
        .gambar-table th { text-align: center; font-size: 14px; padding: 5px; border: 1px solid black; font-weight: bold; }
        .gambar-table td .romawi { position: absolute; top: 5px; left: 5px; font-size: 10pt; font-weight: normal; color: rgba(0,0,0,0.3); }
    </style>
</head>
<body>
    ...
    <thead>
        <tr><th>Riwayat Pemeriksaan</th><th>Foto Makroskopis</th></tr>
    </thead>
    <tbody>
        <?php
        $riwayat_pemeriksaan = [
        ...
        if ($totalRiwayat > 0) :
            foreach ($riwayat_pemeriksaan as $pemeriksaan) :
                foreach ($pemeriksaan['data'] as $row) : ?>
                    <tr>
                        ...
                        <?php if ($firstRow) : ?>
                            <td class="foto-makroskopis" rowspan="<?= $totalRiwayat ?>" style="vertical-align:top;">
                                <div style="display:flex; flex-wrap:wrap; gap:1px; justify-content:flex-start;">
                                    <?php if (!empty($foto) && is_array($foto)): ?>
                                        ...
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <div>Tidak ada foto makroskopis.</div>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <?php $firstRow = false; ?>
            ...
            <tr>
                ...
                <td class="foto-makroskopis" style="vertical-align:top;">
                    <div style="display:flex; flex-wrap:wrap; gap:1px; justify-content:flex-start;">
                        <?php if (!empty($foto) && is_array($foto)): ?>
                            <?php foreach ($foto as $f): ?>
                                <div style="flex:0 0 45%; border:1px solid #000; padding:1px; box-sizing:border-box; text-align:center; margin-bottom:1px;">
        ...
        <tr><td colspan="2">${detailMakroskopis}</td></tr>
    </tbody>
</table>
    ...
    <table class="mikroskopis-content-table">
        ...
        <tr><td colspan="2">${detailMikroskopis}</td></tr>
    </table>

    <table class="mutu-table">
        <tr>
            <td>☐ Jaringan terfiksasi merata?</td>
            <td>☐ Pemotongan topos dan merata?</td>
            ...
            <td>☐ Sediaan tanpa bercak jari?</td>
        </tr>
    </table>
    ...
    <script>
        ...
        // Fallback jika afterprint tidak terpanggil (jarang, tapi berjaga-jaga)
        setTimeout(function(){
